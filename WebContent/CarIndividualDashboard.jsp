...
<div class="content-wrapper" ng-app="airDataApp">
	...
	<section class="content-header">
		<h1>
			Dashboard <small>Car Panel</small> <i class="fa fa-car "></i>
		</h1>
						...
						<div class="small-box bg-green">
							...
							<div class="small-week small-box-footer">
								 <label class="btn btn-transparent grey-salsa btn-sm "><span style="color: aqua;font-size: 15px" >{{confirmedCount.countD}}</span>&nbsp;&nbsp;&nbsp;
								  <a href="showIndividualCarReportList?showReportType=BC&filterUptoDate=D"
									...
									<a href="showIndividualCarReportList?showReportType=BC&filterUptoDate=W"
									...
									<a href="showIndividualCarReportList?showReportType=BC&filterUptoDate=M"
			 ...
			 <div class="col-sm-12">
                            <div class="panel panel-default">
                                <div class="panel-heading clearfix">
                                    ...
                                    <ul class="panel-controls" style="margin-top: 2px;">
                                     <li><a href="#" ng-click="refreshCar()" class="panel-refresh"><span class="fa fa-refresh"></span></a></li>
                                            ...
                                            <tbody ng-repeat="name in commonTopFivePojoCarList">
                                                <tr>
                                                    ...
                                                    <td><span class="label label-danger">{{name.status}}</span></td>
                                                     <td><a href="companyCarReports" class="btn btn-default btn-blue">see all reports</a></td>
                                                      <td><a href="companyCarOrders" class="btn btn-default btn-blue">see all Orders </a></td>
                                                </tr> 
                                            </tbody>
                                        </table>
                                    </div>
...
<script>
	...
	app.controller('airDataCtrl',function($scope, $http) {
						...
						$http.get(baseUrl+"getCarBookingList").then(function(response) {
	...
	app.controller('BookingConfirmCtrl',function($scope, $http) {
		...
		$http.get(baseUrl + "getCarAnalysisJson?bstatus=HK").then(function(response) {
	...
	app.controller('OrderPaymentSuccessCtrl',function($scope, $http) {
		var currentUrl = $(location).attr('href');
		var baseUrl = currentUrl.substr(0, currentUrl.lastIndexOf('/') + 1);
		$http.get(baseUrl + "getCarAnalysisJson?pstatus=PS").then(function(response) {
	...
	app.controller('OrderPaymentFailedCtrl',function($scope, $http) {
		var currentUrl = $(location).attr('href');
		var baseUrl = currentUrl.substr(0, currentUrl.lastIndexOf('/') + 1);
		$http.get(baseUrl + "getCarAnalysisJson?pstatus=PF").then(function(response) {
	...
	$http.get(baseUrl+"getCarAnalysisJson?bstatus=ALL&type=D").then(function(response) {
	...
	$scope.getWeeklyConfirmed=function(){
		$http.get(baseUrl+"getCarAnalysisJson?bstatus=ALL&type=W").then(function(response) {
			$('.doughnutDefault').hide();
			$('.doughnutDay').hide();
			$('.doughnutWeek').show();
			$('.doughnutMonth').hide();
  ...
  $scope.getMonthlyConfirmed=function(){
	   $http.get(baseUrl+"getCarAnalysisJson?bstatus=ALL&type=M").then(function(response) {
		   $('.doughnutDefault').hide();
			$('.doughnutDay').hide();
			$('.doughnutWeek').hide();
			$('.doughnutMonth').show();
