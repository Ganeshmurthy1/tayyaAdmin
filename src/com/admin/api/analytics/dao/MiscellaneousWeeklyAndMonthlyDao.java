...
public class MiscellaneousWeeklyAndMonthlyDao {

	public static final Logger logger = Logger.getLogger(MiscellaneousWeeklyAndMonthlyDao.class);
	
	public Map<String, Integer> getMiscellaneousWeeklyOrMonthlyReport(User userSessionObj, Company companySessionObj, List<String> userIdList,String reportType) {
		List<MiscellaneousOrderRow> miscellaneousOrderRows=new ArrayList<>();
		Map<String, Integer> weekMap=new HashMap<>();
		SessionFactory sessionfactory;
		Session session = null;
		Criteria criteria = null;
		Integer i=1;
		List<Integer> idListUser = new ArrayList<>();
		...
		try {
			...
			if (userSessionObj != null && companySessionObj != null) {
				if (userSessionObj.getUserrole_id().isSuperuser()) {
					...
					if (idListUser != null) {
						conjunction.add(Restrictions.in("userId", idListUser));
					}
				...
				miscellaneousOrderRows = criteria.list();
				for (MiscellaneousOrderRow miscellaneousOrderRow : miscellaneousOrderRows) {
					String[] datenTime =miscellaneousOrderRow.getCreatedAt().toString().split(" ");
	...
	public Long getMiscellaneousWeeklyOrMonthlyCancellationCount(User userSessionObj, Company companySessionObj, List<String> userIdList,String reportType) {
		List<MiscellaneousOrderRow> miscellaneousOrderRows=new ArrayList<>();
		Map<String, Integer> weekMap=new HashMap<>();
		SessionFactory sessionfactory;
		Session session = null;
