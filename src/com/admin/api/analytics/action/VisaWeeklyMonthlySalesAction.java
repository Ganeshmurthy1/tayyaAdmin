...
import com.admin.api.analytics.dao.VisaWeeklyAndMonthlyDao;
...
public class VisaWeeklyMonthlySalesAction extends ActionSupport implements SessionAware {
	...
	public static final Logger logger = Logger.getLogger(VisaWeeklyMonthlySalesAction.class);
	VisaWeeklyAndMonthlyDao visaWeeklyAndMonthlyDao = new VisaWeeklyAndMonthlyDao();
	Map<String, Integer> weekMap = new HashMap<>();
	Map<String, Integer> monthMap = new HashMap<>();
	CommonWeeklyMonthlyJsonData visaJsonData = new CommonWeeklyMonthlyJsonData();
	List<MonthlyAndWeeklyJsonCommonPojo> visaMonthlyandWeeklyDataCountList = new ArrayList<>();

	public String fetchVisaWeeklySalesJson() {
		...
		if (companySessionObj == null) {
			...
			try {
				if (userSessionObj != null && companySessionObj != null) {
					...
					weekMap = visaWeeklyAndMonthlyDao.getVisaWeeklyOrMonthlyReport(userSessionObj,
							companySessionObj, userIdList, reportType);
					CancellationCount = visaWeeklyAndMonthlyDao.getVisaWeeklyOrMonthlyCancellationCount(
					...
					for (int j = 0; j < 7; j++) {
						...
						visaMonthlyandWeeklyDataCountList.add(monthlyAndWeeklyJsonCommonPojo);
					}

					if (weekMap != null && weekMap.size() > 0) {
						Set<Entry<String, Integer>> set = weekMap.entrySet();
						for (Entry<String, Integer> entry : set) {
							for (MonthlyAndWeeklyJsonCommonPojo visa : visaMonthlyandWeeklyDataCountList) {
								if (visa.getDate().trim().equalsIgnoreCase(entry.getKey().trim())) {
									visa.setCount(entry.getValue());
									countTotal = countTotal + entry.getValue();
								}

							}
							visaJsonData.setMonthlyWeeklyCommondata(visaMonthlyandWeeklyDataCountList);
							visaJsonData.setTotalCount(countTotal);
							visaJsonData.setCancellationCount(CancellationCount.intValue());
						}
					}
	...
	public String fetchVisaMonthlySalesJson() {
		User userSessionObj = (User) sessionMap.get("User");
		Company companySessionObj = (Company) sessionMap.get("Company");
		String reportType = "M";
		if (companySessionObj == null) {
			...
			try {
				if (userSessionObj != null && companySessionObj != null) {
					...
					monthMap = visaWeeklyAndMonthlyDao.getVisaWeeklyOrMonthlyReport(userSessionObj,
							companySessionObj, userIdList, reportType);
	...
	public CommonWeeklyMonthlyJsonData getVisaJsonData() {
		return visaJsonData;
	}

	public void setVisaJsonData(CommonWeeklyMonthlyJsonData visaJsonData) {
		this.visaJsonData = visaJsonData;
	}
