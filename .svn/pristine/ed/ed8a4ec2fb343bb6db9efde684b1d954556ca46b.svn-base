 ...
 */
/**
 *  This Action class is used receive file from angular and save it in server.
 */
...
public class UploadFile extends ActionSupport implements SessionAware,ServletRequestAware{	
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public static final org.apache.log4j.Logger logger=org.apache.log4j.Logger.getLogger(UploadFile.class);
			...
			if(ServletActionContext.getRequest() instanceof MultiPartRequestWrapper)
			{
				...
				if(fileParameterNames.hasMoreElements()){
					...
					for (UploadedFile cf : files) {
						logger.info("-----UploadFile Action -- called- and --file length...-------"+cf.length());
