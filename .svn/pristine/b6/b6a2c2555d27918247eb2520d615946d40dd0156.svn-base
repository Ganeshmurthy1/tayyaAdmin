	  ...
	  var receivedAmount=$('#'+rowId+'receivedAmount').val();
  	  ...
  	  $('#'+rowId+'loader').show();
  		 $.ajax({
				    method: "POST",
				    url:finalUrl,
				    data: {travelType:'C',rowId:rowId,totInvoiceAmount:totInvoiceAmount,receiveAmount:receivedAmount},
						...
						<div class="table-responsive dash-table">
							...
							<div class="box clearfix">
								<table id="example" class="table table-striped table-bordered">
									...
									<tbody>
										 <s:if test="knockOffPage.carRows !=null && knockOffPage.carRows.size()>0">
									 <s:iterator value="knockOffPage.carRows" status="rowCount">
											<tr>
